...
function UploadProduct({navigation}) {
  ...
  addRandomUser = async () => {
    setloading(true);
    if (
      (Image !== '', title !== '', amount !== '', time !== '', createdBy !== '')
    ) {
      const Url = await (await uploadImage(Image, title))
      console.log(Url)
      ...
      firestore()
        ...
        addProducts(Url ,title,amount , time , createdBy, user.uid , ids)      
        .then(() => {
